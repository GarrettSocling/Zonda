...
from cirsoc.excepciones import ErrorLineamientos
...
def calcular_cp_cr(cps, areas, area_componente):
    ...
    :param tuple cp: ``tuple`` con dos valores de cp entre los que
        interpolar.
    :param tuple areas: ``tuple`` con dos valores de área entre los que
        interpolar.
    :param float area: El valor de área a utilizar para encontrar el valor de
...
class ParedesComponentes:
    ...
    .. note:: Todos los parámetros numéricos deben ser positivos.
    """
    def __init__(self, altura_media, angulo_cubierta, componentes=None):
...
class CubiertaDosAguasPlanaComponentes:
    ...
    def distancia_a(self):
        """Calcula la distancia de "a" provista en las figuras para componentes
        y revestimientos.

        :rtype: float
        """
        menor_dimension_horizontal = min(self.ancho, self.longitud)
        valor_a_propuesto = min(
            0.1 * menor_dimension_horizontal, 0.4 * self.altura_media
        )
        limite_minimo = max(0.04 * menor_dimension_horizontal, 1)
        return max(valor_a_propuesto, limite_minimo)
...
class Paredes:
    ...
    def __init__(self, ancho, longitud, altura_media, angulo_cubierta,
        ...
        self.componentes = ParedesComponentes(
            altura_media, angulo_cubierta, componentes
        )
...
class Edificio:
    ...
    :param float altura_media: La altura media de cubierta del edificio.
    :param float angulo: El ángulo de cubierta del edificio.
