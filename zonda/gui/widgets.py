...
from gui import ruta_iconos, ruta_imagenes
from . import excepciones, dialogos
import cirsoc
...
class WidgetViento(QtWidgets.QWidget):
    def __init__(self):
        ...
        ruta = os.path.join(ruta_imagenes, 'viento', 'mapa.png')
...
class WidgetTopografia(QtWidgets.QWidget):
    ...
    def _cambio_tipo_terreno(self):
        ...
        if tipo_terreno == 'escarpa bidimensional':
            ...
            imagen = 'loma.png'
        ruta = os.path.join(ruta_imagenes, 'topografia', imagen)
        pixmap = QtGui.QPixmap(ruta)
...
class WidgetCartel(QtWidgets.QWidget):
    def __init__(self):
        ...
        datos_spinboxs = (
            ('altura_superior', 1, 300, 10, ' m'),
            ('altura_inferior', 0, 200, 0, ' m'),
            ('ancho', 1, 300, 5, ' m'),
            ('profundidad', 1, 10, 1, ' m')
        )
        self._spinboxs = {}
        ...
        self._imagen = QtWidgets.QLabel()
        self._imagen.setFrameStyle(QtWidgets.QFrame.StyledPanel)
        imagen = os.path.join(ruta_imagenes, 'estructuras', 'cartel.png')
...
class WidgetCubiertaAislada(QtWidgets.QWidget):
    ...
    def _cambio_tipo_cubierta(self):
        ...
        ruta = os.path.join(
            ruta_imagenes, 'estructuras', 'aislada', tipo_cubierta + '.png'
        )
...
class WidgetEdificio(QtWidgets.QWidget):
    ...
    def _cambio_tipo_cubierta(self):
        ...
        ruta = os.path.join(
            ruta_imagenes, 'estructuras', 'edificio', tipo_cubierta + '.png'
        )
        self._imagen.setPixmap(QtGui.QPixmap(ruta))
...
class WidgetEstructuras(QtWidgets.QWidget):
    ...
    def cirsoc_calculo(self):
        estructuras = (cirsoc.Edificio, cirsoc.Cartel, cirsoc.CubiertaAislada)
...
class WidgetCirsoc(QtWidgets.QTabWidget):
    ...
    def enviar_reporte(self):
        ...
        if estructura is not None:
            try:
                reporte = estructura.reporte()
            except cirsoc.excepciones.ErrorLineamientos as error:
                QtWidgets.QMessageBox.warning(self, 'Advertencia', str(error))
            self.reporte_actualizado.emit(reporte.html())
...
class WidgetReporte(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        for formato, funcion in formatos_funciones:
            ...
            pixmap = QtGui.QPixmap(
                os.path.join(ruta_iconos, formato.lower() + '.svg')
            )
    ...
    def exportar_html(self):
        ...
        if nombre:
            self._vista_web.page().toHtml(partial(self._guardar, nombre))
    ...
    def exportar_txt(self):
        ...
        if nombre:
            self._vista_web.page().toPlainText(partial(self._guardar, nombre))
...
class WidgetPrincipal(QtWidgets.QWidget):
    ...
    def __init__(self, parent):
        ...
        pixmap = QtGui.QPixmap(
            os.path.join(ruta_iconos, 'calculator-solid.svg')
        )
        icono = QtGui.QIcon(pixmap)
